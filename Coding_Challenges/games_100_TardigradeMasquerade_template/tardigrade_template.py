...
def compute_entanglement(theta):
    ...
    @qml.qnode(dev)
    def no_tar():
        qml.MottonenStatePreparation(np.array([0,0.5 ** 0.5,0.5 ** 0.5,0]),wires=[0,1])
        return qml.density_matrix([1])
    # QHACK #
    
    @qml.qnode(dev)
    def tar():
        qml.MottonenStatePreparation(np.array([0,(0.5 ** 0.5)*np.sin(theta/2),(0.5 ** 0.5)*np.cos(theta/2),0,0.5 ** 0.5,0,0,0]),wires=[0,1,2])
        return qml.density_matrix([1])

    return second_renyi_entropy(no_tar()),second_renyi_entropy(tar())
