...
def distance(A, B):
    ...
    def circuit():
        ...
        qml.MottonenStatePreparation([A_f[0]*B_f[0],A_f[0]*B_f[1],A_f[1]*B_f[0],A_f[1]*B_f[1]],wires=[1,2])
        #qml.MottonenStatePreparation(B_f,wires=[2])
    ...
    A_f = np.array([A[0],A[1]])
    A_f = A_f/np.linalg.norm(A_f)

    B_f = np.array([B[0],B[1]])
    B_f = B_f/np.linalg.norm(B_f)


    return np.sqrt(2*(1-np.sqrt(circuit())))
    # QHACK #
